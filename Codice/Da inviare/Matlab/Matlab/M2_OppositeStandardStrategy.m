...
%% STANDARD STRATEGY WITH OPPOSITE FINAL TRANSFER ORBIT

% This matlab script shows and calculates the opposite strategy to transfer
% between two given orbits. The strategy is defined as follows:
% - Change of plane;
% - Change of Periapsis argument;
% - Bitangent transfer starting from the apoapsis and arriving at the periapsis 
% 
...
TransferOrbit(2) = abs(rta-rtp)/(rta+rtp);
TransferOrbit(3) = CurrentOrbit(3);
TransferOrbit(4) = CurrentOrbit(4);
TransferOrbit(5) = CurrentOrbit(5) + pi;
TransferOrbit(6) = 0;
TransferOrbit(7) = pi;
...
Vac = sqrt(mu/pCurrent)*(1-CurrentOrbit(2));
Vpf = sqrt(mu/pFinal)*(1+FinalOrbit(2));
